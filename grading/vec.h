...
template<class T, int n> struct vec; //struct template declaration
template<class T, int n> T dot(const vec<T,n>& u,const vec<T,n>& v); //dot function

template<class T, int n> //struct definition
struct vec
{
    T x[n]; //array in type T

    vec() //constructor
    {make_zero();}

    explicit vec(const T& a) //(a)
    {assert(n == 1);x[0]=a;}

    vec(const T& a, const T& b) //(a,b)
    {assert(n == 2);x[0]=a;x[1]=b;}

    vec(const T& a, const T& b, const T& c) //(a,b,c)
